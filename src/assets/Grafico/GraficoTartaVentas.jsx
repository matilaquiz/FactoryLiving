...
const GraficoTartaVentas = () => {
  const { mes, anio } = useContext(GaficoVentasContext);
    ...
    const traerGraficos2 = async () => {
      try {
        const resp = await axios.get(
          `http://localhost:3000/graficoTortaVentas/${mes}/${anio}`
        );

        const prod = resp.data.map((p) => p.nombre_producto);
        const vendidos = resp.data.map(
          (p) => (p.cantidad_vendida * 100) / p.cantidad_total_mes
        );
