...
const LineasBasicas2 = () => {
    const {material}=useContext(GaficoMaterialesContext)
   ...
   useEffect(() => {
        const traerGraficos2 = async () => {
            try {
                const resp = await axios.get(`http://localhost:3000/graficoAumentoXMP/${material}`)
                ...
                const precio = resp.data.map(mp =>parseFloat(mp.precio_promedio_mensual))
