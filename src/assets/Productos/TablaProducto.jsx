...
useEffect(() => {
    const fetchProductos= async () => {
      try {
        const response = await axios.get("http://localhost:3000/traerProductos");
        ...
        <TableBody className="tablacuerpo">
          {listadoProductos.map((producto) => (
            ...
            >
              ...
              <TableCell align="center">
                <a href="#" onClick={()=>modificarProducto(producto.IdProducto)}>
                  <ModeEdit />
                </a>
              </TableCell>
              <TableCell align="center">
                <a href="#" onClick={()=>confirmarEliminarProducto(producto.IdProducto,producto.Nombre)}>
                  <DeleteForever />
                </a>
              </TableCell>
            </TableRow>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close" onClick={cancelarEliminarProducto}></button>
              </div>
              <div className="modal-body">
                <p>{`¿Está seguro que desea eliminar el producto ${nombreElegido}?`}</p>
              </div><br />
              <div className="modal-footer">
                <button type="button" className="btn btn-secondary"id="botonaceptar" data-bs-dismiss="modal" onClick={cancelarEliminarProducto}>Cancelar</button>
                <button type="button" className="btn btn-primary" id="botoncancelar" onClick={() => eliminarProducto(idEliminar)}>Eliminar</button>
              </div>
            </div>
          </div>
        </div>
